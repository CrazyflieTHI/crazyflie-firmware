...
} __attribute__((packed)) gtPacketacket;
...
// Ground truth to client
static uint8_t groundTruth = 0;
static TickType_t groundTruthFreq = 50;
...
void locSrvInit()
{
  ...
  crtprosRegisterPortCB(CRTP_PORT_LOCALIZATION, locSrvCrtpCB);
  isInit = true;
}
...
{
  gtPacketacket *rp = (gtPacketacket *)pkRange.data;
  ...
  if (enableRangeStreamFloat)
  {
    ...
    if (rangeIndex >= 5)
    {
      ...
      pkRange.size = sizeof(gtPacketacket);
      // This is best effort, i.e. the blocking version is not needed
...
PARAM_ADD_CORE(PARAM_UINT8, sendGt, &groundTruth)
PARAM_ADD_CORE(PARAM_UINT8, gtFreq, &groundTruthFreq)
